...
@pytest.mark.parametrize("api,method,request_data,assert_data,headers,store_var", [("/s-park/statistics/api/vehicle/getTodayInout","POST",{'parkExternalId': '529'},{'success': True, 'result.enterParkVehicleNumber.total': '${>}0', 'result.inParkVehicleNumber.total': '${>}0', 'result.leaveParkVehicleNumber.total': '${>}0'},{'content_type': 'application/json;charset=UTF-8'},{})])
def test_case_1(sample_fixture, api, method, request_data, assert_data, headers, store_var):
    print('开始执行测试用例')
